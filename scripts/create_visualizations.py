...
# ============================================================================
epochs = [1, 2, 3]
baseline_em_epochs = [34.0, 49.7, 52.2]
baseline_f1_epochs = [42.80, 59.21, 61.26]
cartography_em_epochs = [34.1, 54.2, 57.1]
cartography_f1_epochs = [43.59, 63.63, 66.34]

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
fig.suptitle('Training Dynamics: Model Performance Progression Over Epochs', 
             fontsize=14, fontweight='bold', y=1.02)
...
# Add value labels
for x, y in zip(epochs, baseline_em_epochs):
    ax1.text(x, y - 1.5, f'{y:.1f}%', ha='center', fontsize=9)
for x, y in zip(epochs, cartography_em_epochs):
    ax1.text(x + 0.15, y + 1.5, f'{y:.1f}%', ha='center', fontsize=9)
...
# Add value labels
for x, y in zip(epochs, baseline_f1_epochs):
    ax2.text(x, y - 1.5, f'{y:.2f}%', ha='center', fontsize=9)
for x, y in zip(epochs, cartography_f1_epochs):
    ax2.text(x + 0.15, y + 1.5, f'{y:.2f}%', ha='center', fontsize=9)
